...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_agnostic_multistat_forpaper_hill_alt2.tif",sep="")
    tiff(tifoutpath,
  ...
  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/fst/hill70_myPar_log10_bigrange_huge_2par_500range_positive_expfit.txt")
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression('log'[10]*'(F'['ST']*') Q-Q plot'),xlab=expression('Exponential distribution fit to log'[10]*'(F'['ST']*')'),ylab=expression('log'[10]*'(F'['ST']*')'),pch=20,fit=fit)
