...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_walvee_fst_norm.tif",sep="")
    tiff(tifoutpath,
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('F'['ST']*' Q-Q plot','for WAL vs. EE')),xlab=expression('Normal distribution fit to F'['ST']),ylab=expression('F'['ST']),pch=20,fit=fit)
