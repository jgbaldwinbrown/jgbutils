...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_agnostic_multistat_forpaper_hill_alt3.tif",sep="")
    tiff(tifoutpath,
       width=3*5*600, height=1*5*600, res=600, compression="lzw")
  ...
  mydata <- read.table("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/gene_association/fst_significance_data/fst_only.txt",
                     header=TRUE)

  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/fst/hill70_myPar_log10_bigrange_huge_2par_500range_positive.txt")
  tempmyxtx <- log10(mydata$fst_mean+0.01)
  if (min(tempmyxtx) <= 0) {
    myxtx = tempmyxtx - min(tempmyxtx) + .0000001
  } else {
    myxtx = tempmyxtx
  }
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression('log'[10]*'(F'['ST']*') Q-Q plot'),xlab=expression('Gamma distribution fit to log'[10]*'(F'['ST']*')'),ylab=expression('log'[10]*'(F'['ST']*')'),pch=20,fit=fit)
  #plot(theory,exp,main=expression('F'['ST']*' Q-Q plot'),xlab=expression('Gamma distribution fit to F'['ST']),ylab=expression('F'['ST']))
  #abline(fit)
  #grid()
  #rug(theory,side=1)
  #rug(exp,side=2)
  ...
  #######
  #xtx:
  mydata <- read.table("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/bf_multiwin/dsbig_fused_xtxonly_final.txt",
                     header=TRUE)

  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/xtx/hill70_myPar_15it_log10_bigrange_huge_2par_500range_positive.txt")
  myxtx <- log10(mydata$xtx)
  myxtx70 <- getQuantileMini(myxtx,0.1,0.8)
  smyxtx <- sort(myxtx)
  Nf = length(myxtx)
  N=length(myxtx)
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression('log'[10]*'(X'^'T'*'X) Q-Q plot'),xlab=expression('Gamma distribution fit to log'[10]*'(X'^'T'*'X)'),ylab=expression('log'[10]*'(X'^'T'*'X)'),pch=20,fit=fit)

  #plot(theory,exp,main=expression('X'^'T'*'X Q-Q plot'),xlab=expression('Gamma distribution fit to X'^'T'*'X'),ylab=expression('X'^'T'*'X'))
  #abline(fit)
  #grid()
  #rug(theory,side=1)
  #rug(exp,side=2)
  ...
  #######
  #xtx25:
  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/xtx25/hill70_myPar_15it_log10_bigrange_huge_2par_500range_positive.txt")
  myxtx <- log10(mydata$xtx_win25)
  myxtx70 <- getQuantileMini(myxtx,0.1,0.8)
  smyxtx <- sort(myxtx)
  Nf = length(myxtx)
  N=length(myxtx)
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression('log'[10]*'(X'^'T'*'X) Q-Q plot (25-SNP window)'),xlab=expression('Gamma distribution fit to log'[10]*'(X'^'T'*'X)'),ylab=expression('log'[10]*'(X'^'T'*'X)'),pch=20,fit=fit)
