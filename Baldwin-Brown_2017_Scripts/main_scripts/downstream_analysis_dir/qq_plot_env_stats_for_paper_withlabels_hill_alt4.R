...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_env_multistat_forpaper_hill_alt4.tif",sep="")
    tiff(tifoutpath,
  ...
  mydata <- read.table("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/bf_multiwin/dsbig_fused_bf2only_final.txt",
                     header=TRUE)

  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/bf/hill70_myPar_loglog10_bigrange_huge_2par_500range_positive_date.txt")
  tempmyxtx <- log10(log10(mydata$bf2)+0.76)
  if (min(tempmyxtx) <= 0) {
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('log'[10]*'(log'[10]*'(Bayes factor)) for','collection date Q-Q plot')),xlab=expression('Gamma distribution fit to log'[10]*'(log'[10]*'(BF))'),ylab=expression('log'[10]*'(log'[10]*'(BF))'),pch=20,fit=fit)
  ...
  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/lfmm/hill70_myPar_minnolog_bigrange_huge_2par_500range_positive.txt")
  tempmyxtx <- -log10(mydata$adjusted.p.values)
  if (min(tempmyxtx) <= 0) {
    myxtx = tempmyxtx - min(tempmyxtx) + .0000001
  } else {
    myxtx = tempmyxtx
  ...
  N=length(myxtx)
  #theory=qgamma((1:N)/N - 1/(2*Nf),myPar[1],myPar[2])
  theory=sort(-log10(seq(0.0000001,1,length.out=length(smyxtx))))
  exp=smyxtx
  N10 = round(N * .1)
  N80 = round(N*.8)
  theory70 = theory[N10:N80]
  exp70 = exp[N10:N80]
  fit <- lm(exp70~theory70)
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('LFMM -log'['10']*'(p-value) for','pool surface area Q-Q plot')),xlab=expression('-log'['10']*'(Uniform distribution (0,1))'),ylab=expression('-log'['10']*'(p)'),pch=20,fit=fit)
