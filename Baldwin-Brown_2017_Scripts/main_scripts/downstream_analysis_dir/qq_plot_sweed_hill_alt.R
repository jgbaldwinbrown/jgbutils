...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_sweed_multistat_forpaper_hill_alt.tif",sep="")
    tiff(tifoutpath,
       width=1*5*600, height=1*5*600, res=600, compression="lzw")
  ...
  mydata <- readRDS("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/sweed_out/v2_complete/out_combo/sweed_full_allchroms_sorted.RDS")

  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/sweed/hill70_myPar_log10_bigrange_huge_2par_500range_positive.txt")
  tempmyxtx <- log10(mydata$Likelihood+0.000000001)
  if (min(tempmyxtx) <= 0) {
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression('SweeD log'[10]*'(likelihood) Q-Q plot'),xlab=expression('Gamma distribution fit to log'[10]*'(likelihood)'),ylab=expression('log'[10]*'(likelihood)'),pch=20,fit=fit)
  #plot(theory,exp,main=expression('SweeD likelihood Q-Q plot'),xlab=expression('Gamma distribution fit to likelihood'),ylab=expression('Likelihood'))
  #abline(fit)
  #grid()
  #rug(theory,side=1)
  #rug(exp,side=2)
