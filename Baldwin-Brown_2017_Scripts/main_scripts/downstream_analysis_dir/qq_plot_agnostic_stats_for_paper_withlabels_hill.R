...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_agnostic_multistat_forpaper_hill.tif",sep="")
    tiff(tifoutpath,
  ...
  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/fst/hill70_myPar_bigrange_huge_2par_500range_positive.txt")
  myxtx <- mydata$fst_mean
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression('F'['ST']*' Q-Q plot'),xlab=expression('Gamma distribution fit to F'['ST']),ylab=expression('F'['ST']),pch=20,fit=fit)
  ...
  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/xtx/hill70_myPar_15it_bigrange_huge_2par_500range_positive.txt")
  myxtx <- mydata$xtx
  myxtx70 <- getQuantileMini(myxtx,0.1,0.8)
  smyxtx <- sort(myxtx)
  Nf = length(myxtx)
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression('X'^'T'*'X Q-Q plot'),xlab=expression('Gamma distribution fit to X'^'T'*'X'),ylab=expression('X'^'T'*'X'),pch=20,fit=fit)
