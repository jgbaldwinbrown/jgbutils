...
setLayout <- function(xdim=1,ydim=1){
  ...
  myheights <- rep(c(1.5/10,1/10,7.5/10),ydim)
...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_ltervt011_log10_fst_expnorm.tif",sep="")
    tiff(tifoutpath,
  ...
  mydata <- readRDS("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/walvee_and_ltervt011/python_fst/only-PASS-Q30-SNPs-cov_v2_sorted_ltervt011_fst_cens_multiwin_fst/only-PASS-Q30-SNPs-cov_v2_sorted_ltervt011_fst_cens_multiwin_snpnames_chromnums.RDS")

  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/fst/hill70_myPar_ltervt011_exp.txt")
  myxtx <- log10(mydata$fst_0_1+0.028)
  myxtx70 <- getQuantileMini(myxtx,0.1,0.8)
  smyxtx <- sort(myxtx)
  Nf = length(myxtx)
  N=length(myxtx)
  theory=qexp((1:N)/N - 1/(2*Nf),myPar[1])
  exp=smyxtx
  N10 = round(N * .1)
  N80 = round(N*.8)
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('log'['10']*'(F'['ST']*')'*' Q-Q plot','for LTER vs. Tank011')),xlab=expression('Exponential distribution fit to log'['10']*'(F'['ST']*')'),ylab=expression('log'['10']*'(F'['ST']*')'),pch=20,fit=fit)
  ...
  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/fst/hill70_myPar_ltervt011_25win_norm.txt")
  myxtx <- log10(mydata$fst_0_1_win25+0.018)
  myxtx70 <- getQuantileMini(myxtx,0.1,0.8)
  smyxtx <- sort(myxtx)
  Nf = length(myxtx)
  N=length(myxtx)
  theory=qnorm((1:N)/N - 1/(2*Nf),myPar[1],myPar[2])
  exp=smyxtx
  N10 = round(N * .1)
  N80 = round(N*.8)
  theory70 = theory[N10:N80]
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('log'['10']*'(F'['ST']*')'*' Q-Q plot','for LTER vs. Tank011 (25-SNP window)')),xlab=expression('Normal distribution fit to log'['10']*'(F'['ST']*')'),ylab=expression('log'['10']*'(F'['ST']*')'),pch=20,fit=fit)
...
myin_fst <- "/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/walvee_and_ltervt011/python_fst/only-PASS-Q30-SNPs-cov_v2_sorted_ltervt011_fst_cens_multiwin_fst/only-PASS-Q30-SNPs-cov_v2_sorted_ltervt011_fst_cens_multiwin_snpnames_chromnums.RDS"
