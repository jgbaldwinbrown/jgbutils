...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_sweed_multistat_forpaper_hill.tif",sep="")
    tiff(tifoutpath,
  ...
  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/sweed/hill70_myPar_bigrange_huge_2par_500range_positive.txt")
  myxtx <- mydata$Likelihood
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression('SweeD likelihood Q-Q plot'),xlab=expression('Gamma distribution fit to likelihood'),ylab=expression('Likelihood'),pch=20,fit=fit)
