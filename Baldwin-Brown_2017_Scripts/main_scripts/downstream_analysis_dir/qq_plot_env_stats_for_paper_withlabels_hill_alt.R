...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_env_multistat_forpaper_hill_alt.tif",sep="")
    tiff(tifoutpath,
  ...
  myPar <- scan("/home/jbaldwin/new_home/wild_shrimp_data/for_shrimp_paper_1/bwa_alignments/snp_tables/deduped/new_downsampled/full_fst_dsbig_fused/bayenv_5repavg/v4_full_data_fixedinput_splitout/hill_climber_outs/bf/hill70_myPar_loglog10_bigrange_huge_2par_500range_positive.txt")
  tempmyxtx <- log10(log10(mydata$bf23)+0.6)
  if (min(tempmyxtx) <= 0) {
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('log'[10]*'(log'[10]*'(Bayes factor)) for','pool surface area Q-Q plot')),xlab=expression('Gamma distribution fit to log'[10]*'(log'[10]*'(BF))'),ylab=expression('log'[10]*'(log'[10]*'(BF))'),pch=20,fit=fit)
  ...
  exp70 = exp[N10:N80]
  fit <- lm(exp70~theory70)
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('LFMM -(p-value) for','pool surface area Q-Q plot')),xlab=expression('Gamma distribution fit to -(p)'),ylab=expression('-(p)'),pch=20,fit=fit)
