...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_walvee_fst_expnorm.tif",sep="")
    tiff(tifoutpath,
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('F'['ST']*' Q-Q plot','for WAL vs. EE')),xlab=expression('Exponential distribution fit to F'['ST']),ylab=expression('F'['ST']),pch=20,fit=fit)
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('F'['ST']*' Q-Q plot','for WAL vs. EE (25-SNP window)')),xlab=expression('Normal distribution fit to F'['ST']),ylab=expression('F'['ST']),pch=20,fit=fit)
