...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_ltervt011_fst_gamma.tif",sep="")
    tiff(tifoutpath,
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('F'['ST']*' Q-Q plot','for LTER vs. Tank011')),xlab=expression('Gamma distribution fit to F'['ST']),ylab=expression('F'['ST']),pch=20,fit=fit)
  ...
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('F'['ST']*' Q-Q plot','for LTER vs. Tank011 (25-SNP window)')),xlab=expression('Gamma distribution fit to F'['ST']),ylab=expression('F'['ST']),pch=20,fit=fit)
