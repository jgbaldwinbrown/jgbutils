...
class snp(object):
    def __init__(self):
        ...
        self.xtxhits_fbgn = []
    def print_all(self):
        outlist = [self.name,self.chrom,self.pos,self.xtx,unk_list(self.xtxhits_sg),unk_list(self.xtxhits_fbgn)]
        print "\t".join(map(str,outlist))
...
def print_snp_header():
    outlist = ["snp_name","chrom","pos","XtX_score","shrimp_genes_hit_xtx","dmel_genes_hit_xtx"]
    print "\t".join(outlist)
...
xtx_sig_table = "gene_association/mean_XtX_out.normalized_transposed_tank_info_11pop_xtxsigfilter.txt"
region_sig_snps_file = "gene_association/significant_xtx_snps_associated_with_genes_13regions.txt"
...
region_sig_snps = set([x.split("\t")[0] for x in open(region_sig_snps_file,"r")][1:])

#print("split1")
for line in open(xtx_sig_table,"r"):
    ...
    #print score
    #snps[name].print_all()
    if name not in region_sig_snps:
        continue
    for genekey,gene in data.iteritems():
        for hit in gene.hit_extents:
            ...
            if hit[1] == snps[name].chrom and hit[2] - 5000 <= snps[name].pos <= hit[3] + 5000:
                snps[name].xtxhits_sg.append(hit[0])
                try:
                    snps[name].xtxhits_fbgn.append(sgv2_2_fbgn[hit[0]])
                except KeyError:
                    pass
    #debug:
    #snps[name].print_all()
...
for key,mysnp in snps.iteritems():
    if len(mysnp.xtxhits_sg) > 0 or len(mysnp.xtxhits_fbgn) > 0:
        mysnp.print_all()
