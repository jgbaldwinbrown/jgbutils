...
qqplot_fst_lfmm_xtx_bf <- function(inpath_fst,inpath_xtx,inpath_bf,inpath_sweed,inpref_lfmm,insuf_lfmm,outpref,npops,wins,zs,bfs,fsts,
  ...
  tifoutpath = paste(outpref,"_env_multistat_forpaper_hill_alt3.tif",sep="")
    tiff(tifoutpath,
  ...
  tempmyxtx <- -mydata$adjusted.p.values
  if (min(tempmyxtx) <= 0) {
    myxtx = tempmyxtx - min(tempmyxtx) + .0000001
  } else {
    myxtx = tempmyxtx
  ...
  N=length(myxtx)
  #theory=qgamma((1:N)/N - 1/(2*Nf),myPar[1],myPar[2])
  theory=seq(0,1,length.out=length(smyxtx))
  exp=smyxtx
  N10 = round(N * .1)
  N80 = round(N*.8)
  theory70 = theory[N10:N80]
  exp70 = exp[N10:N80]
  fit <- lm(exp70~theory70)
  scatterBarLay(cbind(theory,exp),bigmain=expression(atop('LFMM 1-(p-value) for','pool surface area Q-Q plot')),xlab=expression('Uniform distribution (0,1)'),ylab=expression('1-(p)'),pch=20,fit=fit)
