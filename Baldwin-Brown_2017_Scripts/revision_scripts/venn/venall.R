...
main <- function() {
    ...
    colnames(venndata) = c("HC FST", "HC-BP XTX", "UG-BE2 XTX", "UG FST")
    vc = vennCounts(venndata)

    plotscale = 8
    pdf("venn.pdf", width=plotscale, height=plotscale)
    vennDiagram(vc)
    dev.off()
    ...
    colnames(venndata2) = c("HC-BP XTX, Sig", "HC-BP XTX", "UG-BE2 XTX", "UG-BE2 XTX, Sig")
    vc2 = vennCounts(venndata2)

    plotscale = 10
    pdf("venn2.pdf", width=plotscale, height=plotscale)
    vennDiagram(vc2)
    dev.off()
