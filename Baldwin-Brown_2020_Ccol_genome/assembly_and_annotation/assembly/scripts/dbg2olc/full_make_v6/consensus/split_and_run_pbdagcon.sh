...
for file in $(ls ${split_dir}/*.reads.fasta); do
    ...
    #echo $chunk
    cmd="blasr -nproc MYCORES ${split_dir}/${chunk}.reads.fasta ${split_dir}/${chunk}.fasta -bestn 1 -m 5 -minMatch 19 -out ${split_dir}/${chunk}.mapped.m5"
    echo $cmd
    eval $cmd

    cmd="pbdagcon ${split_dir}/${chunk}.mapped.m5 -j MYCORES -c 1 -m 200 > ${split_dir}/${chunk}.consensus.fasta"
    echo $cmd
    eval $cmd
