...
paths=temp/clean_data/500bp/trimmomatic/500bp_trimmed_to_fqjoin.txt
...
outdir=temp/clean_data/180bp/fqjoin
mkdir -p outdir
outprefix=`basename ${in1f} .txt.gz`

fastq-join ${in1f} ${in2f} -o ${outdir}/${outprefix}.%.fq 1> ${outdir}/fqjoin_tigershrimp_out.txt 2> ${outdir}fqjoin_tigershimp_err.txt
