...
def plot_combo(combo, outname, mids):
    ...
    scale = 1.5
    pdf(outname, height=4*scale, width=20*scale)
...
def main():
    ...
    for family in families:
        repdata = get_data("Repeats", "family_densities/repanswer_family_" + family + ".bed", "reflens.txt", offset)
        combo = combine_data(genedata, repdata)
        chrom_mids = get_chrom_mids(combo[["Scaffold", "Start", "End", "Scaffold_number", "plotpos"]], offset)
        print(combo.head())
        plot_combo(combo, "combo_" + family + ".pdf", chrom_mids)
