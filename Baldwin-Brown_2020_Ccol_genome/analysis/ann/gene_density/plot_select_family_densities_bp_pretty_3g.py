...
def plot_combo(combo, outname, mids):
    ...
    pdf(outname, height=3*4.6*scale, width=20*scale)
        aplot = ggplot(data = data, aes(plotpos, true_density)) +
            ...
            theme_bw() +
            scale_y_continuous(labels = function(x) sprintf("%.2g", x)) +
            ...
            theme(text = element_text(size=48), panel.spacing = unit(1.8, "lines"), plot.margin = unit(c(.5,.5,.5,.5), "cm"), axis.text.y = element_text(hjust=0))
        print(aplot)
    dev.off()
...
def main():
    ...
    print(combo.head())
    plot_combo(combo, "select_families_dens_pretty_3g.pdf", chrom_mids)
