...
def plot_combo(combo, outname, mids):
    ...
    scale = 2
    pdf(outname, height=2.5*4*scale, width=20*scale)
...
def main():
    offset = 5000000
    genedata = get_data("Genes", "answer.bed", "reflens.txt", offset)
    with open("repeat_supergenus_counts_semiselect.txt", "r") as inconn:
        families = [x.rstrip('\n').split('\t')[-1] for x in inconn]
        repdata = get_data("Repeats", "repanswer.bed", "reflens.txt", offset)
    datas = []
    for family in families:
        datas.append(get_data(family, "family_densities/repanswer_family_" + family + ".bed", "reflens.txt", offset))
    ...
    print(combo.head())
    plot_combo(combo, "select_families.pdf", chrom_mids)
