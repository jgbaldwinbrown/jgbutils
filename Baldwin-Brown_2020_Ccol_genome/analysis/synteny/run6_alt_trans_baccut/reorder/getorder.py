...
def get_biggest_hit(pchrom_data):
    counter = {}
    for hit in pchrom_data:
        cchrom = hit[0]
        pstart = hit[5]
        pend = hit[6]
        pspan = abs(pend - pstart)
        if cchrom not in counter:
            counter[cchrom] = [0, []]
        counter[cchrom][0] += pspan
        counter[cchrom][1].append(hit)
    sorted_counter = sorted(counter.items(), key = lambda x: x[1][0], reverse=True)
    return(sorted_counter[0][1][1][0])

def subset_hits(data):
    out = {}
    for pchrom_name, pchrom_data in data.items():
        entry = get_biggest_hit(pchrom_data)
