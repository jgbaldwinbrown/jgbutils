...
data$indiv_chrom_tissue = paste(data$indiv, data$tissue, sep="_")

head(data)
write.table(data, txt_out)

datameans = aggregate(data$diff, list(data$indiv, data$tissue), mean)
colnames(datameans) = c("indiv", "tissue", "x")
datameans$indiv_chrom_tissue = paste(datameans$indiv, datameans$tissue, sep="_")
head(datameans)

a = sapply(datameans$indiv_chrom_tissue,
    function(temp){
        t.test(abs(data$diff[data$indiv_chrom_tissue == temp]),
            abs(data$diff[data$tissue=="blood"]))$p.value
    }
)
datameans$p = a
...
ggplot(data=datameans[as.numeric(as.character(datameans$indiv))<=10,], aes(tissue, x)) + geom_bar(stat="identity") + facet_grid(indiv~.) + ggtitle(pdf_title)
dev.off()

pdf(pdf_out2,height=10,width=3)
ggplot(data=datameans[as.numeric(as.character(datameans$indiv))<=10,], aes(tissue, -log10(p))) + geom_bar(stat="identity") + facet_grid(indiv~.) + ggtitle(pdf_title2)
dev.off()
