...
ggplot(data=datameans, aes(tissue, x)) + geom_bar(stat="identity") + facet_grid(indiv~.) + ggtitle(pdf_title)
dev.off()

pdf(pdf_out2,height=10,width=3)
ggplot(data=datameans, aes(tissue, -log10(p))) + geom_bar(stat="identity") + facet_grid(indiv~.) + ggtitle(pdf_title2)
dev.off()
