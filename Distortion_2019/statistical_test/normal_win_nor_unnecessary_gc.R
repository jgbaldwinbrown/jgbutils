...
chrom_sliding_window = function(indiv_chrom_tissue, indiv, chrom, tissue, control_tissue, data, win_size, win_step) {
    rows_to_keep1 = (data[,"indiv_chrom_tissue"] == indiv_chrom_tissue) 
...
str(data)
l = lm(data, formula = freq_nor ~ pos + gc*indiv*tissue)
str(l)
...
rolldat = as.data.frame(
    sapply(
        levels(
            factor(data$indiv_chrom_tissue)
        ),
        function(x){
            chrom_sliding_window(x,
                data$indiv[data$indiv_chrom_tissue==x][1],
                data$chrom[data$indiv_chrom_tissue==x][1],
                data$tissue[data$indiv_chrom_tissue==x][1],
                control_tissue,
                data,
                win_size,
                win_step)
